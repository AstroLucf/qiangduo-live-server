...
const PTS = {
  ...
  // ★fanjoin(加入粉丝团) 故意【不在这张表里】★
  //   ptsOf 返回 0 → ledger.record 开头 `if (!unit) return 0` 直接跳过，一分不记、不进积分池。
  //   这不是漏写：《直播玩法准入规则》规定「外显变化」与「战果影响」只能二选一，我们选了外显。
  //   给它任何分值都会同时踩两条线 —— 合规上变成战果影响，工程上白送 52 抖币等值的钱进池子被瓜分。
  //   ⚠ 别为了"走通用流程"给它补一行，那正好会把计分/推力/小火箭一起打开。
};
