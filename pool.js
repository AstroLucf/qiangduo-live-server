...
const ROUND_CAP = 200;
// ⚠️ 【死代码·2026-08-06 起无人调用】入场名次冻结榜曾经跟着主播落盘 ——
//   后来名次榜改成【全平台一份】(ledger.js 的 worldSnap)，不再按主播存。
//   唯一调用方 ledger.savePool 现在只传 4 个参数，第 5 个 rankSnap 恒为 undefined；
//   `get()` 返回的 rankSnap 也没有任何读取点。存量记录里残留的 rs 字段下次 set 会自然清掉。
//   留着不删是因为删了要动 set 的签名；**别照它的注释以为名次还按主播存**。
// ★（历史说明）当年为什么要落盘：★
