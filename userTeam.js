...
// sides = 该主播房间的落座表（room.side）。不传 → 退回模块级兜底表（老调用/自查工具）。
function queryUserGroup(rawBody, round, sides) {
  const body = pickBody(rawBody);
  const side = dy.chosenSide(body.open_id || '', sides);
  return {
...
function userGroupPush(rawBody, round, broadcast, worldRankOf, room) {
  ...
  if ((side === 'left' || side === 'right') && typeof broadcast === 'function') {
    ...
    noteInteraction(user, broadcast, worldRankOf, false, !prev, room);
  }
  return {
    errcode: 0, errmsg: 'success',
...
function audienceChange(rawBody, broadcast, worldRankOf, room) {
...
// ============================================================
// ★2026-08-05 多直播间隔离：去重表改成【由调用方注入】（rooms.js 的 room.entered / room.seen）。
//   原来是模块级单份 —— A 房给某观众播过入场视频，他去 B 房就不播了；
//   而且 A 房开局 resetRoundEnter() 会把 B 房的记录一起清掉。
//   ⚠ 不传 room = 退回下面这份兜底（本地/自查工具/老包），行为与改造前一致。
const enteredThisRound = new Set();           // 本局已【真的播过】入场的 openid（兜底）
const seenThisRound = new Set();              // 本局出现过的 openid（判首次 + 防日志刷屏）（兜底）
const ENT = (room) => (room && room.entered instanceof Set ? room.entered : enteredThisRound);
const SEEN = (room) => (room && room.seen instanceof Set ? room.seen : seenThisRound);
const COOL = (room) => (room && room.cooldown instanceof Map ? room.cooldown : rankEnterCooldown);
function resetRoundEnter(room) { ENT(room).clear(); SEEN(room).clear(); }

// isGift=true 表示本次互动是送礼（会改变世界榜名次）→ 需要重新查名次。
function noteInteraction(user, broadcast, worldRankOf, isGift, isJoin, room) {
  ...
  if (ENT(room).has(openId)) return;          // 本局已播过 → 后续互动不再触发
  const first = !SEEN(room).has(openId);
  SEEN(room).add(openId);
